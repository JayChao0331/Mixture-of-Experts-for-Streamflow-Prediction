...
class _SamplingSetup():
    ...
    def __init__(self, model: 'BaseModel', data: Dict[str, torch.Tensor], head: str):
        ...
        dropout_rates = {"model": model.dropout.p}
        for name in ["embedding_net", "forecast_embedding_net", "hindcast_embedding_net"]:
            embedding = getattr(model, name, None)
            if embedding is not None:
                dropout_rates[f"{name}.statics"] = embedding.statics_embedding_p_dropout
                dropout_rates[f"{name}.dynamics"] = embedding.dynamics_embedding_p_dropout

        max_implied_dropout = max(dropout_rates.values())
        if cfg.mc_dropout and not 0.0 < max_implied_dropout < 1.0:
            raise RuntimeError(f"{cfg.model} with `mc_dropout` activated requires the maximum dropout rate "
                               f"to be between 0 and 1 (exclusive). Current rates: {dropout_rates}")
