...
def get_head(cfg: Config, n_in: int, n_out: int, activation: Optional[str] = None) -> nn.Module:
    ...
    activation : str, optional
        Override the configured output activation for a regression head.
    ...
    if cfg.head.lower() == "regression":
        head = Regression(n_in=n_in, n_out=n_out,
                          activation=cfg.output_activation if activation is None else activation)
