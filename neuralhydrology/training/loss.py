...
class MaskedMSELoss(BaseLoss):
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        ...
        loss = torch.mean((prediction['y_hat'][mask] - ground_truth['y'][mask])**2)
...
class MaskedMSELoss_Regularized(BaseLoss):
    """Mean squared error loss.
    ...
    def __init__(self, cfg: Config):
        super(MaskedMSELoss_Regularized, self).__init__(cfg, prediction_keys=['y_hat', 'y_peak', 'y_normal', 'y_low'], ground_truth_keys=['y'])
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        ...
        # Mixture Loss
        loss_mixture = torch.mean((prediction['y_hat'][mask] - ground_truth['y'][mask])**2)
        
        sigmoid_values = torch.sigmoid(ground_truth['y'][mask])
        weight_peak = 1 + sigmoid_values
        weight_low = 2 - sigmoid_values

        # Peak Loss
        loss_peak = torch.mean(weight_peak * ((prediction['y_peak'][mask] - ground_truth['y'][mask])**2))

        # Normal Loss
        loss_normal = torch.mean((prediction['y_normal'][mask] - ground_truth['y'][mask])**2)

        # Low Loss
        loss_low = torch.mean(weight_low * ((prediction['y_low'][mask] - ground_truth['y'][mask])**2))

        loss = loss_mixture + loss_peak + loss_normal + loss_low


        # Plot Weighted Function Curve
        # self.x_vals.append(gt_values.cpu())
        # self.y_vals.append(1 + sigmoid_values.cpu())

        # if ground_truth['y'][mask].size(0) < 256:
        #     # Concatenate all
        #     self.x_vals = torch.cat(self.x_vals).numpy()
        #     self.y_vals = torch.cat(self.y_vals).numpy()

        #     # Sort for smooth curve
        #     sorted_indices = self.x_vals.argsort()
        #     x_sorted = self.x_vals[sorted_indices]
        #     y_sorted = self.y_vals[sorted_indices]

        #     # Plot
        #     plt.figure(figsize=(8, 4))
        #     plt.scatter(self.x_vals, self.y_vals, s=10, alpha=0.5, label='Weights')
        #     plt.plot(x_sorted, y_sorted, color='orange', linewidth=1.2, label='Sigmoid curve')
        #     plt.xlabel('Discharge Value')
        #     plt.ylabel('Weight')
        #     plt.title('Weighting Curve for Low Discharge Values')
        #     plt.grid(True)
        #     plt.legend()
        #     plt.tight_layout()
        #     plt.savefig("./sigmoid_transformed_discharge.png")
        #     plt.close()
...
class PeakMSELoss(BaseLoss):
    def __init__(self, cfg: Config):
        super(PeakMSELoss, self).__init__(cfg, prediction_keys=['y_hat'], ground_truth_keys=['y'])
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        ...
        # Peak Loss
        loss = torch.mean(weight_peak * ((prediction['y_hat'][mask] - ground_truth['y'][mask])**2))

        return loss


class LowMSELoss(BaseLoss):
    def __init__(self, cfg: Config):
        super(LowMSELoss, self).__init__(cfg, prediction_keys=['y_hat'], ground_truth_keys=['y'])
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        ...
        # Peak Loss
        loss = torch.mean(weight_low * ((prediction['y_hat'][mask] - ground_truth['y'][mask])**2))
...
class MaskedWeightedMSELoss(BaseLoss):
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        ...
        error = torch.abs(prediction['y_hat'][mask] - ground_truth['y'][mask])
        weights = (1 + error).pow(self.cfg.gamma)

        loss = weights * ((prediction['y_hat'][mask] - ground_truth['y'][mask])**2)
...
#         # Compute the top 10% and bottom 10% thresholds
#         top_threshold = torch.quantile(y_true, 0.9)
#         bottom_threshold = torch.quantile(y_true, 0.1)

#         # Define weights based on thresholds
#         peak_weight = 2.0  # Higher weight for peak values
#         low_weight = 2.0   # Higher weight for low values
#         normal_weight = 1.0  # Default weight for other values

#         # Create weight tensor based on conditions
#         weights = torch.ones_like(y_true) * normal_weight
#         weights[y_true >= top_threshold] = peak_weight  # Assign peak weight
#         weights[y_true <= bottom_threshold] = low_weight  # Assign low weight

#         # Calculate the weighted MSE loss
#         error = prediction['y_hat'][mask] - y_true
#         loss = 0.5 * weights * (error ** 2)
#         loss = torch.mean(loss)
