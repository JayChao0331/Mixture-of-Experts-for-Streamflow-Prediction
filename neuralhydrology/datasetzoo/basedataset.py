...
class BaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.id_to_int = dict(id_to_int) if cfg.use_basin_id_encoding else {}
        self.scaler = scaler
        ...
        if self.is_train and self.cfg.distributed_rank == 0:
            self._dump_scaler()
    ...
    def _create_id_to_int(self):
        ...
        if self.cfg.distributed_rank != 0:
            return
    ...
    def _load_or_create_xarray_dataset(self) -> xarray.Dataset:
        ...
        if (self.cfg.train_data_file is None) or (not self.is_train):
            ...
            if self.is_train and self.cfg.save_train_data and self.cfg.distributed_rank == 0:
    ...
    def _create_lookup_table(self, xr: xarray.Dataset):
        ...
        for basin in tqdm(basin_coordinates, file=sys.stdout, disable=self._disable_pbar):
            ...
            for freq in self.frequencies:
                ...
                # print("\n")
                # print("x_d: {}".format(x_d[freq][0]))
                # print("y: {}".format(y[freq][0]))
                # print("date: {}".format(dates[freq][0]))
                # print("\n")
                # print("x_d: {}".format(x_d[freq][1]))
                # print("y: {}".format(y[freq][1]))
                # print("date: {}".format(dates[freq][1]))
                # print("\n")
                # print("x_d: {}".format(x_d[freq][2]))
                # print("y: {}".format(y[freq][2]))
                # print("date: {}".format(dates[freq][2]))
                # print("\n")
    ...
    def boxcox_transform_xarray(self, xr_data, variable_name):
        epsilon = 1e-6

        # Extract the data as a 1D array, ignoring NaNs
        flat_data = xr_data[variable_name].values.flatten()
        ...
        transformed_data, lambda_opt = boxcox(flat_data)

        # Reshape the transformed data back to the original shape
        reshaped_data = np.full_like(xr_data[variable_name].values, np.nan)  # Start with NaNs
        reshaped_data[~np.isnan(xr_data[variable_name].values)] = transformed_data

        # Update the xarray Dataset
        xr_data[variable_name] = (xr_data[variable_name].dims, reshaped_data)

        print(f"Optimal lambda for Box-Cox transformation: {lambda_opt}")
        return xr_data, lambda_opt
    ...
    def _load_data(self):
        ...
        # Apply log transform (handle NaN and ensure no log(0) or negative values)
        # if self.period != "test":
        #     epsilon = 1e-6
        #     xr["discharge"] = xr["discharge"].where(xr["discharge"].isnull(), np.log(xr["discharge"] + epsilon))
        #     xr, optimal_lambda = self.boxcox_transform_xarray(xr, "discharge")
        ...
        # print("\n")
        # print("xr after transform: {}".format(xr["discharge"]))
        # print("\n")
        ...
        # # Loop through each basin in the xarray
        # for basin in xr["basin"].values:
        #     # Extract the discharge data for the current basin
        #     basin_data = xr["discharge"].sel(basin=basin).values.flatten()
            
        #     # Remove NaN values
        #     basin_data = basin_data[~np.isnan(basin_data)]
            
        #     # Create the distribution plot
        #     sns.displot(basin_data, kde=True, bins=30, color='blue', height=5, aspect=1.5)
            
        #     # Add title and labels
        #     plt.title(f"Distribution of Discharge for Basin {basin}", fontsize=14)
        ...
        #     # Save the figure as a PNG file with proper DPI
        #     plt.savefig(
        #         os.path.join(output_dir, f"basin_{basin}_discharge_distribution.png"),
        ...
        # Apply Power Transform after Z-Score Normalization
        # if self.period != "test":
        #     p = 1.2
        #     normalized_discharge = xr["discharge"].values
        #     transformed_discharge = np.sign(normalized_discharge) * np.abs(normalized_discharge) ** p
        #     xr["discharge"].values = transformed_discharge
